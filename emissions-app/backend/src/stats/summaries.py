...
class DailySummary:
    ...
    def __init__(self, date: dt.date, individual_emissions: list[DailyIndividual]):
        self.date = date
        self.total_recorded_count = len(individual_emissions)
        self.total_recorded_co2_kg = sum(
            emissions.co2_emissions_kg for emissions in individual_emissions
        )
        self.total_recorded_distance_km = sum(
            emissions.distance_km for emissions in individual_emissions
        )
    ...
    def __init__(self, daily_summaries: list[DailySummary]):
        if len(daily_summaries) >= 7:
            raise LongerThanWeekException(
                "Too many daily summaries; cannot compute a week - filter beforehand"
            )
        self.total_recorded_count = sum(
            summary.total_recorded_count for summary in daily_summaries
        )
        self.total_recorded_co2_kg = sum(
            summary.total_recorded_co2_kg for summary in daily_summaries
        )
        self.total_recorded_distance_km = sum(
            summary.total_recorded_distance_km for summary in daily_summaries
        )
        self.avg_co2_kg_per_record = average(
            self.total_recorded_co2_kg, self.total_recorded_count
        )
